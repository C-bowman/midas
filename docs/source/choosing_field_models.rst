...
objects also specify their required parameters through a ``parameters`` instance attribute
...
For example, we can model the 1D profile of a field as a Gaussian function using the
following field model:
...
.. code-block:: python
    ...
    class GaussianField(FieldModel):
        ...
        def get_values_and_jacobian(
            ...
            dz_dc = -1.0 / parameters[self.width]
            # build the jacobians dictionary
            jacobians = {
                self.amplitude: g,
                self.centre: parameters[self.amplitude] * dg_dz * dz_dc,
                self.width: parameters[self.amplitude] * dg_dz * dz_dw,
            }
